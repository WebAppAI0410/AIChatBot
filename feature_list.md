...
| Notes | テキスト部分選択AIアシスト | 選択範囲のみの編集・整形、アクションバブル |
...
## ノート管理
- フォルダ階層でノート・フォルダを管理（PCのドキュメント管理風UI）
- ノート一覧画面に右下FAB（新規ノート/フォルダ作成、タブUIと重ならない配置）
- ノートエディタはApple純正ノート風（タップで編集モード、入力は自動保存、保存ボタンなし）
- WYSIWYGエディタはWebView（TipTap v2/TOAST UI Editor）
- テキスト選択時のアクションバブル（選択範囲のみAI編集・整形）
- タグ付与・編集・削除・一括編集・タグ名変更
- AIアシスト（ノート編集画面：質問/編集/部分モード切替、ノート一覧画面：全体管理・一括タグ編集）
- すべての作成/編集/削除は自動保存
- クラウド同期はv1では不要
