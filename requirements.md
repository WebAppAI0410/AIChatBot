...
## 5. 主な非機能要件
- **ローカルモデル DL** : Unsloth版Qwen3-4B (Q4_K_M) 4bit GGUF（約2.5GB）、Wi‑Fi推奨、途中再開対応。コンテキスト長32,768トークン対応。メモリ不足時はQ3_K_M（約2.08GB）へ自動フォールバック。
- **ストレージ** : インストール容量残 < 3GB 時は警告。llama.rnによるネイティブ推論。
