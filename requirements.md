...
- **UI/UX** : チャット一覧では左スワイプで削除アクションを表示、削除前に確認ダイアログを表示し、誤操作を防止する。
- **チャット編集** : チャット一覧で各チャットのタイトル右に編集アイコン（えんぴつ）を表示。タップでインライン編集モードに切り替え、タイトルを編集可能。
- **チャットルーム画面でも、上部タイトル部分をタップしてインライン編集できる。タイトルは2行まで表示可能。
- **サムネ編集** : チャットごとにサムネ（アイコン）を設定可能。デフォルトアイコン20種＋端末画像から選択可。
- **新規チャット作成** : 新規チャット作成画面で「現在のモデル: 〇〇」表示をタップするとモデル選択モーダルが開き、作成時に任意のモデルを指定できる。
