...
// ブレイクポイントに基づいてスペーシングを取得するヘルパー関数
function getSpacingByBreakpoint(breakpoints: any, theme: any) {
  if (breakpoints.sm) return theme.spacing.sm;
  if (breakpoints.md) return theme.spacing.md;
...
export const useResponsive = () => {
  ...
  return useMemo(() => {
    ...
    const breakpoints = {
      sm: width < BREAKPOINTS.SM_MAX,
      md: width >= BREAKPOINTS.MD_MIN && width < BREAKPOINTS.MD_MAX,
      lg: width >= BREAKPOINTS.LG_MIN,
      xl: width >= BREAKPOINTS.XL_MIN,
    };
    
    // デバイスタイプを画面幅に基づいて判定
    // 注: 各デバイスは必ず1つのカテゴリにのみ分類されるよう設計されています
    // 例: 幅1050pxの画面は isTablet=false, isDesktop=true となります
    // breakpointの値を変更する場合は、このロジックが維持されるよう注意してください
    const deviceType = {
    ...
    const spacing = {
      container: getSpacingByBreakpoint(breakpoints, theme),
      card: getSpacingByBreakpoint(breakpoints, theme),
