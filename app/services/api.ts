...
// Supabase Storage Edge FunctionのURL
const SUPABASE_STORAGE_FUNCTION_URL = 'https://alperyqhdtpnivxfnqdi.supabase.co/functions/v1/get-signed-url';
...
class Api {
  ...
  async getSignedUrl(path: string): Promise<string> {
    try {
      const response = await fetch(`${SUPABASE_STORAGE_FUNCTION_URL}?path=${encodeURIComponent(path)}`, {
        method: 'GET',
        headers: {
  ...
  async fetchChatCompletion(
    ...
    try {
      ...
      const actualModelId = this.normalizeModelId(modelId);
...
// 後方互換性のために関数版もエクスポート
export const fetchChatCompletion = (
  messages: ChatMessage[],
  modelId: string,
  onChunk?: (chunk: string) => void
): Promise<string> => {
  return api.fetchChatCompletion(messages, modelId, onChunk);
};

// ヘルパー関数のエクスポート
export const normalizeModelId = (modelId: string): string => {
  return api.normalizeModelId(modelId);
};
