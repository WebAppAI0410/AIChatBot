  ...
  const handleItemPress = useCallback((item: { type: 'folder' | 'note', id: string, name?: string, title?: string }) => {
    if (item.type === 'folder') {
      ...
      if (newFolderId === item.id) {
        // handleFolderNameSubmit(); // 一旦コメントアウト
      }
      navigateToFolder(item.id);
    } else {
      router.push(`/notes/${item.id}`); // ノート編集画面への遷移を有効化
    }
  }, [navigateToFolder, newFolderId, router]); // 依存配列を修正
  
  // 新規ノート作成
  const handleCreateNote = useCallback(() => {
    router.push('/notes/new'); // 新規ノート作成画面への遷移を有効化
  }, [router]); // 依存配列を修正
